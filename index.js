...
async function downloadAndSchedule(url, chatId, messageId) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            await safeEdit(bot, chatId, messageId, "Downloading video... (0%)");
...
// Simple menu without special chars
const menu = {
    reply_markup: {
        keyboard: [
            ['Status', 'Schedule', 'Latest'],
            ['Send Link', 'Zero Views', 'Refresh'],
            ['Help', 'Test Bot']
        ],
...
bot.command('test', async (ctx) => {
    ...
    if (fs.existsSync(TEMP_DIR)) {
        statusMsg += `✅ Temp directory ready\n`;
    } else {
...
bot.help(async (ctx) => {
    await safeReply(ctx,
        ...
        `3. Adds watermark: ${WATERMARK_TEXT}\n` +
        `4. Schedules to your channel (7 days)\n\n` +
        `Zero-view videos auto re-upload after 2 hours\n\n` +
        `Use /test to check if bot is working`
    );
});
...
bot.hears('Latest', async (ctx) => {
    ...
    if (latest) {
        ...
        await safeReply(ctx, `Could not fetch latest post. Check channel ID.`);
    }
});
...
bot.hears('Zero Views', async (ctx) => {
    if (zeroViewVideos.size === 0) {
        await safeReply(ctx, `No zero-view videos detected. All your videos have views!`);
    } else {
...
bot.on('text', async (ctx) => {
    ...
    const buttons = ['Status', 'Schedule', 'Latest', 'Send Link', 'Zero Views', 'Refresh', 'Help', 'Test Bot'];
    ...
    if (urlPattern.test(text)) {
        ...
        if (match) {
            ...
            if (activeDownloads.has(ctx.chat.id)) {
                await safeReply(ctx, `Please wait, a download is already in progress! Let it finish first.`);
                return;
            }
...
bot.catch((err, ctx) => {
    ...
    safeReply(ctx, `Error: ${err.message}\n\nPlease try again or use /test to diagnose.`);
});

...
def _handle_payment_webhook():
    ...
    if not tx_ref:
        print(f"⚠️ Webhook missing tx_ref: {data}")
    ...
    try:
        ...
        if v.status_code != 200:
            print(f"⚠️ Chapa verify HTTP {v.status_code}: {v.text[:200]}")
            return jsonify({"ok": True, "ignored": True})
        verified = v.json().get("data", {})
        if verified.get("status") != "success":
            print(f"⚠️ Chapa verify status: {verified.get('status')}")
            return jsonify({"ok": True, "ignored": True})
    except Exception as e:
    ...
    for phone, user in USERS.items():
        ...
        if pay.get("pending_tx_ref") == tx_ref:
            ...
            save_users()
            print(f"✅ Payment verified & activated: {phone} until {pay['until']}")
            break
...
def admin_get_user_payment(phone):
    ...
    return jsonify({
        ...
        "state": _user_subscription_state(user),
    })
...
def admin_update_settings():
    data = request.get_json() or {}
    for k in ["default_price_etb", "default_period_days",
              "default_trial_days", "payments_enabled_globally"]:
        if k in data:
            SETTINGS[k] = data[k]
    save_settings()
